#!/bin/bash

CURRENT_DIRECTORY="01_unseal"
# Auto unseal
sudo bash -c "cat >/root/$CURRENT_DIRECTORY/s1_reconfig.sh" <<EOT
#!/bin/bash
BLACK="\033[0;30m"
BLUE="\033[0;34m"
GREEN="\033[0;32m"
GREY="\033[0;90m"
CYAN="\033[0;36m"
RED="\033[0;31m"
PURPLE="\033[0;35m"
WHITE="\033[1;37m"
COLOR_RESET="\033[0m"

clear
cat <<DESCRIPTION
We're adding a new "seal" stanza to our config file:

seal "awskms" {
    region = "${AWS_REGION}"
    kms_key_id = "00000000-0000-0000-0000-000000000000"
}

Press any key to continue...
DESCRIPTION

read -n1 kbd

cat >>/etc/vault.d/vault.hcl <<VAULTCFG

seal "awskms" {
    region = "${AWS_REGION}"
    kms_key_id = "${AWS_KMS_KEY_ID}"
}
VAULTCFG

echo "Restarting Vault..."
sleep 5
service vault restart
echo "Done"
echo "Getting status..."
sleep 3
vault status
EOT
chmod a+x /root/$CURRENT_DIRECTORY/s1_reconfig.sh

sudo bash -c "cat >/root/$CURRENT_DIRECTORY/s2_unseal_migrate.sh" <<EOT
#!/bin/bash
BLACK="\033[0;30m"
BLUE="\033[0;34m"
GREEN="\033[0;32m"
GREY="\033[0;90m"
CYAN="\033[0;36m"
RED="\033[0;31m"
PURPLE="\033[0;35m"
WHITE="\033[1;37m"
COLOR_RESET="\033[0m"

clear
cat <<DESCRIPTION
Now we need to unseal Vault with migration flag to move the 
key to AWS KMS.

vault operator unseal -migrate $UNSEAL_KEY_1
vault operator unseal -migrate $UNSEAL_KEY_2
vault operator unseal -migrate $UNSEAL_KEY_3

Press any key to continue...
DESCRIPTION

read -n1 kbd

vault operator unseal -migrate $UNSEAL_KEY_1 > /dev/null
vault operator unseal -migrate $UNSEAL_KEY_2 > /dev/null
vault operator unseal -migrate $UNSEAL_KEY_3 > /dev/null
vault status
EOT
chmod a+x /root/$CURRENT_DIRECTORY/s2_unseal_migrate.sh

sudo bash -c "cat >/root/$CURRENT_DIRECTORY/s3_unseal_migrate.sh" <<EOT
#!/bin/bash
BLACK="\033[0;30m"
BLUE="\033[0;34m"
GREEN="\033[0;32m"
GREY="\033[0;90m"
CYAN="\033[0;36m"
RED="\033[0;31m"
PURPLE="\033[0;35m"
WHITE="\033[1;37m"
COLOR_RESET="\033[0m"

clear
cat <<DESCRIPTION
Key shards are no longer necessary. Instead, we have recovery keys.
You can leave the recovery keys as is using Shamir secret sharing,
but we're going to rekey it to just have a single recovery key.

vault operator rekey \\\\
    -init \\\\
    -target=recovery \\\\
    -key-shares=1 \\\\
    -key-threshold=1

Press any key to continue...
DESCRIPTION

read -n1 kbd

vault operator rekey -init -target=recovery -key-shares=1 -key-threshold=1 > /root/$CURRENT_DIRECTORY/rekey.txt

export NONCE_KEY=\$(cat /root/$CURRENT_DIRECTORY/rekey.txt | sed -n '/^Nonce/p' | awk -F " " '{print \$2}')
vault status
EOT
chmod a+x /root/$CURRENT_DIRECTORY/s3_unseal_migrate.sh

sudo bash -c "cat >/root/$CURRENT_DIRECTORY/s4_unseal_rekey.sh" <<EOT
#!/bin/bash
BLACK="\033[0;30m"
BLUE="\033[0;34m"
GREEN="\033[0;32m"
GREY="\033[0;90m"
CYAN="\033[0;36m"
RED="\033[0;31m"
PURPLE="\033[0;35m"
WHITE="\033[1;37m"
COLOR_RESET="\033[0m"

clear
cat <<DESCRIPTION
Finally, we can complete the rekey process using the NONCE token 
generated by the previous command. Using three of the unseal/recovery
keys, we'll finalize rekeying the recovery key(s).

vault operator rekey \\\\
    -target=recovery \\\\
    -key-shares=1 \\\\
    -key-threshold=1 \\\\
    -nonce=\$NONCE_KEY \\\\
    $UNSEAL_KEY_1

vault operator rekey \\\\
    -target=recovery \\\\
    -key-shares=1 \\\\
    -key-threshold=1 \\\\
    -nonce=\$NONCE_KEY \\\\
    $UNSEAL_KEY_2

vault operator rekey \\\\
    -target=recovery \\\\
    -key-shares=1 \\\\
    -key-threshold=1 \\\\
    -nonce=\$NONCE_KEY \\\\
    $UNSEAL_KEY_3

Press any key to continue...
DESCRIPTION

read -n1 kbd

vault operator rekey -target=recovery -key-shares=1 -key-threshold=1 -nonce=\$NONCE_KEY $UNSEAL_KEY_1 > /dev/null
vault operator rekey -target=recovery -key-shares=1 -key-threshold=1 -nonce=\$NONCE_KEY $UNSEAL_KEY_2 > /dev/null
vault operator rekey -target=recovery -key-shares=1 -key-threshold=1 -nonce=\$NONCE_KEY $UNSEAL_KEY_3 > /dev/null

vault write sys/license text=${VAULT_LICENSE} > /dev/null

vault status
EOT
chmod a+x /root/$CURRENT_DIRECTORY/s4_unseal_rekey.sh

sudo bash -c "cat >/root/$CURRENT_DIRECTORY/runall.sh" <<EOT
echo "Configuring auto unseal..."
cat >>/etc/vault.d/vault.hcl <<VAULTCFG

seal "awskms" {
    region = "${AWS_REGION}"
    kms_key_id = "${AWS_KMS_KEY_ID}"
}
VAULTCFG

sleep 5
service vault restart
sleep 3

vault operator unseal -migrate $UNSEAL_KEY_1 > /dev/null
vault operator unseal -migrate $UNSEAL_KEY_2 > /dev/null
vault operator unseal -migrate $UNSEAL_KEY_3 > /dev/null

vault operator rekey -init -target=recovery -key-shares=1 -key-threshold=1 > /root/$CURRENT_DIRECTORY/rekey.txt

export NONCE_KEY=\$(cat /root/$CURRENT_DIRECTORY/rekey.txt | sed -n '/^Nonce/p' | awk -F " " '{print \$2}')

vault operator rekey -target=recovery -key-shares=1 -key-threshold=1 -nonce=\$NONCE_KEY $UNSEAL_KEY_1 > /dev/null
vault operator rekey -target=recovery -key-shares=1 -key-threshold=1 -nonce=\$NONCE_KEY $UNSEAL_KEY_2 > /dev/null
vault operator rekey -target=recovery -key-shares=1 -key-threshold=1 -nonce=\$NONCE_KEY $UNSEAL_KEY_3 > /dev/null

vault write sys/license text=${VAULT_LICENSE} > /dev/null

echo "Done!"
EOT
chmod a+x /root/$CURRENT_DIRECTORY/runall.sh
